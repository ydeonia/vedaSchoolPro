...
final staffLeaveProvider = FutureProvider<List>((ref) async {
  final response = await api.get('/api/mobile/staff/leaves');
  return response.data['leaves'] ?? [];
});

class StaffLeave extends ConsumerStatefulWidget {
  const StaffLeave({super.key});

  @override
  ConsumerState<StaffLeave> createState() => _StaffLeaveState();
}

class _StaffLeaveState extends ConsumerState<StaffLeave> {
  ...
  Future<void> _submitLeave() async {
    ...
    try {
      await api.post('/api/mobile/staff/leave/apply', data: {
      ...
      ref.invalidate(staffLeaveProvider);
      setState(() {
        _showForm = false;
        _reasonCtrl.clear();
        _fromDate = null;
  ...
  Widget build(BuildContext context) {
    final leaves = ref.watch(staffLeaveProvider);
                        ...
                        Expanded(
                          child: GestureDetector(
                            ...
                            child: InputDecorator(
                              ...
                              child: Text(
                                _fromDate != null ? fmt.format(_fromDate!) : 'Select',
                                style: TextStyle(
                                  color: _fromDate != null ? null : theme.textTheme.bodySmall?.color,
                                ),
                              ),
                            ),
                          ),
                        ...
                        Expanded(
                          child: GestureDetector(
                            ...
                            child: InputDecorator(
                              ...
                              child: Text(
                                _toDate != null ? fmt.format(_toDate!) : 'Select',
                                style: TextStyle(
                                  color: _toDate != null ? null : theme.textTheme.bodySmall?.color,
                                ),
                              ),
                            ),
                          ),
              ...
              data: (items) {
                ...
                return Column(
                  children: items.asMap().entries.map((entry) {
                    ...
                    final status = leave['status'] ?? leave['admin_status'] ?? 'pending';
                    return AnimatedCard(
                      index: i + 1,
